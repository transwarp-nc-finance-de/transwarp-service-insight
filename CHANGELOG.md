...
- Chromium 技术复验覆盖默认 3 条、部分命中 2 条与 1 条、无命中、loading、4 类快捷补充、自由输入、继续提交、确定和取消，结果通过；Edge 冒烟、业务口径和最终关闭仍由人工签字。
...
- Chromium 技术复验覆盖默认内部资料、关闭 Wiki 严格隔离、开源资料路由、RAG 关闭后的通用知识降级、全部关闭后的信息不足、配置刷新恢复与对话快照、模拟附件发送和命令复制；未发现控制台错误。Edge 冒烟、来源准入、管理员责任和业务关闭仍由人工确认。
