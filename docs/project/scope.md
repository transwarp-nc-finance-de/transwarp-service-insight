...
状态：ACTIVE，Last reviewed: 2026-07-16。当前实现与一期目标必须分开解读。
...
- 已授权后续独立 Ticket 在受控环境按固定 revision 进行一次取件，并校验固定文件清单与 SHA-256；Issue #19 本身不下载模型。制品不提交 Git，运行时不得联网；受控取件授权不等于允许打包、默认启用或将资格门禁标为 `PASS`。
