...
- Issue #39 已完成固定 revision 的受控取件、固定文件清单与 SHA-256 校验、供应链材料、离线安全、资源性能和固定评估集实测；Issue #19 于 2026-07-19 经人工复核确认为 `PASS`。允许 Issue #25 使用批准制品实施 `local-embedding`、pgvector、双索引与原子发布闭环；制品不得提交 Git，产品构建必须校验批准的逐文件 SHA-256，运行时不得联网或隐式下载。
