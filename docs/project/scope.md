...
- 一期默认 Embedding 为 `intfloat/multilingual-e5-base` revision `d13f1b27baf31030b7fd040960d60d909913633f`，固定 768 维、512 Token 上限及 `query:`/`passage:` 前缀；只加载必要的 Safetensors 与 tokenizer，不执行远程自定义代码。内部非商用使用边界和许可证剩余风险已获人工确认；最终依赖锁/SBOM/NOTICE、资源门槛或固定评估集未通过时不得作为验收默认组件。
- Issue #39 已完成固定 revision 的受控取件、固定文件清单与 SHA-256 校验、供应链材料、离线安全、资源性能和固定评估集实测；Issue #19 于 2026-07-19 经人工复核确认为 `PASS`。Issue #25 已使用批准制品实现 `local-embedding`、pgvector、双索引与原子发布闭环；制品不得提交 Git 或写入镜像，产品启动前必须校验批准的逐文件 SHA-256，运行时不得联网或隐式下载。
...
- 当前 `/api/v1` 字段、语义与行为保持不变；重定义后的一期 Context、知识治理、独立反馈/继续提交、引用和评估能力使用 `/api/v2`。新旧 DTO 通过 Mapper 进入应用用例，领域模型不依赖 HTTP 版本。
