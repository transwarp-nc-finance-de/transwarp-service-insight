...
以下是已确认范围的剩余 Backlog；它只记录能力缺口，不构成实施计划。完整 `/api/v2` OpenAPI 及 v1→v2 映射已获人工批准；API v2 仍为 `DRAFT`，整体实施状态为 `PARTIALLY_IMPLEMENTED`，批准状态为 `APPROVED_FOR_IMPLEMENTATION`，一期实施状态为 `READY_FOR_IMPLEMENTATION`。当前实现为兼容的 `v1 Mock` 加 v2 AuthSession、知识上传/治理/发布、持久化 Precheck Session/Run、授权混合 Retrieval 与 Evidence、独立 Feedback、SubmissionContinuation 和结构化 AuditEvent；Evaluation、Metrics、Admin Reset 等能力仍未完成，不代表一期已经完成。
...
- [ ] Evaluation、Metrics 与 Admin Reset 等剩余一期业务状态持久化。
...
- [x] 默认完整 Compose 四服务健康验收与显式 FTS-only profile。
