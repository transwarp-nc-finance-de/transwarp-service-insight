...
以下是已确认范围的剩余 Backlog；它只记录能力缺口，不构成实施计划。完整 `/api/v2` OpenAPI 及 v1→v2 映射已获人工批准，API v2 状态为 `APPROVED_FOR_IMPLEMENTATION`，一期实施状态为 `READY_FOR_IMPLEMENTATION`。当前实现为兼容的 `v1 Mock` 加 v2 AuthSession/PostgreSQL 基础切片。

- [ ] 本地 Mock 登录/切换、服务端 Session/Cookie、四角色 RBAC 与产品线过滤。
- [ ] PostgreSQL/pgvector 持久化、迁移、模拟初始化、受控重置与原始文件 Compose volume。
- [ ] Markdown/TXT/文本型 PDF 上传、解析预览、草稿修订、职责分离审核、双索引成功后原子发布。
