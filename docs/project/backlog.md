...
以下是已确认但尚未实现的范围 Backlog；它只记录能力缺口，不构成实施计划。完整 `/api/v2` OpenAPI 及 v1→v2 映射已获人工批准，API v2 状态为 `APPROVED_FOR_IMPLEMENTATION`，一期实施状态为 `READY_FOR_IMPLEMENTATION`。当前实现仍为 `v1 Mock`。
