...
> **DRAFT / PARTIALLY_IMPLEMENTED / APPROVED_FOR_IMPLEMENTATION**：本文用于一期本地模拟闭环的契约实施边界。一期实施状态为 `READY_FOR_IMPLEMENTATION`；[`openapi.yaml`](openapi.yaml) 定义的 v1 Mock 继续作为兼容契约。[`openapi-v2.yaml`](openapi-v2.yaml) 已获准进入实施，当前仅实现 `POST /api/v2/auth-sessions`、`GET /api/v2/auth-sessions/current` 和 `DELETE /api/v2/auth-sessions/current`。其余 v2 operation 均为 `NOT_IMPLEMENTED`，当前切片不代表一期完成，也不接真实身份、生产数据库或外部系统。所有示例语义均为 `模拟数据`。
...
当前达到的条件是“API 契约已获人工批准，且 AuthSession 基础切片已实现”，不是“一期已完成”。需求范围为 `CONFIRMED`；API v2 仍为 `DRAFT`，整体实施状态为 `PARTIALLY_IMPLEMENTED`，批准状态为 `APPROVED_FOR_IMPLEMENTATION`，一期实施为 `READY_FOR_IMPLEMENTATION`。当前实现标识为 `V1_MOCK_PLUS_V2_AUTH_SESSION`；v1 Mock 保持兼容，除三条 AuthSession operation 外的其余 v2 operation 均为 `NOT_IMPLEMENTED`。
