...
> **DRAFT / 未实现 / APPROVED_FOR_IMPLEMENTATION**：本文用于一期本地模拟闭环的契约实施边界。一期实施状态为 `READY_FOR_IMPLEMENTATION`；当前唯一实现仍是 [`openapi.yaml`](openapi.yaml) 定义的 v1 Mock。[`openapi-v2.yaml`](openapi-v2.yaml) 已获准进入实施，但不代表 Mapper、Controller、身份、持久化、知识导入、检索或评估能力已经存在。所有示例语义均为 `模拟数据`。
...
| 无 | `POST /api/v2/auth-sessions` + `GET /api/v2/auth-sessions/current` | v2 本地模拟 Cookie Session 通过响应头发放或恢复绑定当前 Session 的 CSRF Token；v1 没有认证或 CSRF 语义，不得反向映射。 |
| 无 | `GET /api/v2/knowledge-versions/{versionId}/parse-preview` 及 `/blocks`、`/chunks` | v2 提供受对象权限保护的解析摘要、规范化文本块与 Chunk 独立分页预览；固定按 `sequence ASC`，v1 没有等价资源。 |
...
| 待补充信息 | 本地目录种子值的最终清单；未来 v1 下线策略（当前不设日期）；真实 AIOps 字段与附件标识映射。 |

当前达到的条件是“API 契约已获人工批准，可进入实施”，不是“API 已实现”或“一期已完成”。需求范围为 `CONFIRMED`，API v2 为 `APPROVED_FOR_IMPLEMENTATION`，一期实施为 `READY_FOR_IMPLEMENTATION`；当前实现仍为 `v1 Mock`。
