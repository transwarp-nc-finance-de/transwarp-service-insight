...
- Mapper 是 v1/v2 DTO 与应用用例之间的隔离边界；已标记 `IMPLEMENTED` 的 v2 operation 运行时存在，但这不允许服务端把 v1 请求静默升级为 v2。
...
| 无 | `POST /api/v2/knowledge-documents` + `GET /api/v2/parse-tasks/{taskId}` | **已实现**。仅支持模拟 Markdown/TXT/文本型 PDF 的不可变本地文件、草稿版本和异步解析任务；双索引发布由独立 v2 publication/IndexTask operation 承载，授权在线检索由 Precheck Run 和 Evidence operation 承载。 |
...
| 无 | `POST /api/v2/knowledge-versions/{versionId}/revisions`、`/review-submissions`、`/draft-returns`、`/approvals` | **已实现**。提供不可变草稿修订、职责分离送审、退回和批准，以及不可变审核历史；不包含发布或索引。 |
| 无 | `POST/GET /api/v2/feedback`、`POST/GET /api/v2/submission-continuations`、`GET /api/v2/audit-events` | **已实现**。Feedback 与 Continuation 使用独立事务、幂等键和失败边界；AuditEvent 数据库级不可变并按 ADMIN 产品线授权返回脱敏结构化元数据。 |
| 无 | Evaluation、Metrics、Admin Reset 及其他仅存在于 v2 的资源 | Evaluation、Metrics 与 Admin Reset 已按 operation 标记 `IMPLEMENTED`；其他资源仍以 operation 级状态为准，不得从 v1 是否存在推断运行时可用。 |
...
| 已实现 `openapi.yaml` | v1 零变更；本文逐项说明非兼容映射 | 已覆盖；v1 仍是唯一完整已实现契约，v2 目前实现 AuthSession、知识上传解析预览与不可变审核切片。 |
...
当前达到的条件是“API 契约已获人工批准，且 AuthSession、知识闭环、持久化预诊、Evidence、Feedback、Continuation 与结构化审计切片已实现”，不是“一期已完成”。需求范围为 `CONFIRMED`；API v2 仍为 `DRAFT`，整体实施状态为 `PARTIALLY_IMPLEMENTED`，批准状态为 `APPROVED_FOR_IMPLEMENTATION`，一期实施为 `READY_FOR_IMPLEMENTATION`。v1 Mock 保持兼容，未明确标记为 `IMPLEMENTED` 的 v2 operation 仍为 `NOT_IMPLEMENTED`。
