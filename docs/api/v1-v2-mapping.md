...
| 无 | `POST /api/v2/knowledge-documents` + `GET /api/v2/parse-tasks/{taskId}` | **已实现**。仅支持模拟 Markdown/TXT/文本型 PDF 的不可变本地文件、草稿版本和异步解析任务；双索引发布由独立 v2 publication/IndexTask operation 承载，在线检索仍未实现。 |
