...
Source of truth for: 已实现 v1 与未实现 v2 候选契约之间的字段、状态、错误及迁移边界

> **DRAFT / 未实现 / NOT_READY**：本文只用于一期本地模拟闭环的人工契约评审。当前唯一已实现 API 契约仍是 [`openapi.yaml`](openapi.yaml)；[`openapi-v2.yaml`](openapi-v2.yaml) 不代表 Mapper、Controller、身份、持久化、知识导入、检索或评估能力已经存在。所有示例语义均为 `模拟数据`。
...
| 无 | v2 知识、任务、Evidence、评估、指标、审计、策略和本地重置资源 | 全部是未实现 DRAFT 能力，不得从 v1 是否存在推断运行时可用。 |
...
| 解析、索引、评估、重置任务 | 无 | `PENDING / RUNNING / SUCCEEDED / FAILED` | 任务最大总尝试次数为 3；历史失败任务不可变。 |
...
| 未显式区分权限失败 | `INSUFFICIENT_ROLE` | CSRF 失败可使用 `CSRF_VALIDATION_FAILED`。 |
...
- v2 本地身份使用服务端 `SESSION` Cookie；所有 Cookie 写操作要求 `X-CSRF-Token`。
...
| 事实来源 | v2 落点 | 自审结果 |
| --- | --- | --- |
| `docs/project/scope.md`：一期本地模拟闭环、三 Run、终态与幂等 | Session/Run Schema、创建与冲突响应 | 已覆盖；实现前仍须人工批准契约。 |
...
| 已实现 `openapi.yaml` | v1 零变更；本文逐项说明非兼容映射 | 已覆盖，v1 仍是唯一已实现契约。 |
...
| 置信度 | `MEDIUM`：DRAFT 已形成并通过结构校验，但尚未获得 API、产品与安全负责人的人工批准。 |
| 人工介入建议 | API 负责人审查字段/状态/错误与幂等；产品负责人确认目录值与可选字段；安全负责人确认 Cookie/CSRF、职责分离和安全 404。批准前不得开始依赖 v2 的实现。 |
| 待补充信息 | v2 契约是否批准；本地目录值的最终清单；未来 v1 下线策略（当前不设日期）；真实 AIOps 字段与附件标识映射。 |

达到的条件是“API 契约可进入人工评审”，不是“API 已实现”或“一期已 READY”。人工批准前整体状态保持 `NOT_READY`。
