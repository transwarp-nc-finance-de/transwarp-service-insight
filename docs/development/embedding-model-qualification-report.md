...
- 生产批量索引可接受的完整重建窗口；
- 真实知识语料、真实生产负载和多并发容量；
- 是否将约 19 小时 39 分钟的全量资格矩阵保留为专项/夜间门禁，或拆分为快速与完整两层门禁。
...
| 资格镜像 | `sha256:543b6f6db320a236e95f8cdd69fbdb33e1f1da2185f7903456ef5a73eaec8b87` |
| 镜像大小 | `325,026,235` bytes |
| 依赖锁 hash | `sha256:031ef06bbf4c2803e686887db99a172c6c5e9cea644a57a73bf7faa63f39356f` |
| SBOM hash | `sha256:b6c497992f6d173b0de6ced1b25dec206c684f9d138ab5c5099d8780a44d5b17` |
| dataset | `mock-eval-v1` |
| dataset checksum | `sha256:315f364835d88023c6225c62b6feff95cf2df1fd3fe26fb4fe92048e26aa9229` |
| 测量 Git commit | `ac3be49c7826a0cfb1660915ea99cfbe77c407ba` |
...
- 模型目录只读挂载，运行镜像不含下载凭据、Git/LFS、Hugging Face 下载工具或自定义模型 Python 文件。
...
预检记录显示当时没有其他运行容器，可用内存 `14,074,216,448` bytes、CPU 负载 33%。最终性能期间未并行执行 Maven、npm、Compose、其他模型容器或 Agent 构建。

## 冷启动与内存

| 运行 | 加载秒数 | 首次推理 ms | 首次推理 RSS bytes | cgroup peak bytes |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 5.487 | 1,280.519 | 999,936,000 | 1,151,889,408 |
| 2 | 4.013 | 1,314.685 | 1,001,877,504 | 916,729,856 |
| 3 | 4.010 | 1,290.032 | 1,001,492,480 | 916,185,088 |
| 4 | 4.093 | 1,338.590 | 1,002,123,264 | 917,266,432 |
| 5 | 4.026 | 1,265.332 | 1,003,073,536 | 918,216,704 |

平均加载 4.326 秒，平均首次推理 1,297.832 ms。批量矩阵最大进程 RSS `2,072,436,736` bytes，cgroup 峰值 `2,031,345,664` bytes；均低于 4,000,000,000 bytes，未发生 OOM。

## 热态单查询延迟

固定并发 1、batch size 1、`query:` 前缀；每轮先预热 100 次，再记录 1,000 次，使用单调时钟。

| 轮次 | P50 ms | P95 ms | P99 ms | max ms |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 118.558 | 149.476 | 162.891 | 187.195 |
| 2 | 118.901 | 149.305 | 161.214 | 245.852 |
| 3 | 118.708 | 150.096 | 162.952 | 186.274 |

三轮 P95 全部低于 1,000 ms，未用平均值掩盖单轮超限。

## 批量索引基线

36 组矩阵覆盖 100/1,000/10,000 chunks、32/128/512 Token 桶和 batch 1/8/16/32。全部组 `failureCount=0`，最终向量数与输入一致。

- 总测量时间：`70,833.785` 秒；
- chunks/s 范围：`1.022–6.080`；
- tokens/s 范围：`491.405–606.418`；
- 最重组合：10,000 chunks、512 Token、batch 32，用时 `9,535.003` 秒；
- 未设置或推导未经人工确认的吞吐硬门槛。

这组结果只建立当前单核线程配置下的工程基线。全矩阵墙钟约 19 小时 39 分钟，不适合直接作为同步 CI；是否接受完整重建窗口仍需人工确认。
...
| 4 GiB 无 OOM、峰值 <= 4 GB | `PASS` | cgroup peak 2,031,345,664 bytes |
| 三轮热态查询 P95 <= 1,000 ms | `PASS` | 149.476 / 149.305 / 150.096 ms |
| 批量基线完整性 | `PASS` | 36 组、0 失败；没有吞吐硬门槛 |
...
`sha256:f788fc3515095a70a8900fbb87f36edc8d56986b6c3db3753f61a7aa7192a256`

仓库内脱敏副本见 `tools/embedding-qualification/evidence/`。逐请求原始结果仅保存 caseId、语言/场景标签、tokenCount、duration、status、degradation 和模型 manifest hash，不含正文。

## 建议回填 Issue #19 的评论

> Issue #39 已完成固定 revision 的受控资格实测。五个批准文件共 1,129,285,340 bytes，manifest hash 为 `8f58395f...`; 隔离镜像 digest 为 `sha256:543b6f...`。CPU-only、`network=none`、4 GiB 条件下无 OOM，cgroup 峰值 2,031,345,664 bytes；三轮热态查询 P95 为 149.476 / 149.305 / 150.096 ms。36 组批量基线全部成功。`mock-eval-v1` 运行 `SUCCEEDED` 且 `gatePassed=true`：权限泄漏率 0%、引用错误率 0%、降级通过率 100%、Recall@5 96%。模拟数据：小样本工程评估，不代表生产效果。建议人工复核报告、SBOM/NOTICE、模型 manifest 与原始结果 checksum 后决定 Issue #19 的 `PASS/FAIL/BLOCKED`；本评论不自动关闭或标记 Decision Gate。
