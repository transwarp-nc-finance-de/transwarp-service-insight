...
Source of truth for: Issue #19 的候选模型、供应链证据、资格实测协议、`mock-eval-v1` 评估口径与最终人工门禁结论

## 结论摘要

**Issue #19 当前状态：`PASS`（2026-07-19 人工确认，Issue 已关闭）。** 人工已复核 Issue #39、PR #42、资格实测报告、模型 manifest、依赖锁、SBOM、NOTICE、镜像绑定及仓库外原始结果。Issue #25 已据此接入批准的固定制品、`local-embedding`、pgvector、双索引原子发布与默认四服务 Compose；模型文件仍保持 Git/镜像外置，运行时只读挂载并禁止联网或隐式下载。

`intfloat/multilingual-e5-base@d13f1b27baf31030b7fd040960d60d909913633f` 是范围文档已经确认的默认评估对象。Issue #39 已取得下列可审计结论：

- 五个批准文件实际共 `1,129,285,340` bytes，完整 SHA-256 manifest hash 为 `8f58395f4bf0f613fd15ebbf3d5467193ad6717715c4167c764e6e91aae1810e`；
- 最终运行依赖已锁定并生成 SBOM、第三方 NOTICE 和固定镜像 digest；
- 目标 CPU 主机在 4 GiB、CPU-only、`network=none` 下无 OOM，三轮热态查询 P95 为 159.685 / 153.533 / 162.682 ms；
- 32/128/512 精确 Token 桶的 36 组批量索引基线全部成功；`mock-eval-v1` 权限泄漏率 0%、引用错误率 0%、降级通过率 100%、Recall@5 96%。

完整证据、限制、原始结果 checksum 和门禁表见 [Issue #39 资格实测报告](embedding-model-qualification-report.md)。

2026-07-16，用户以本项目唯一开发者及全部责任角色身份，确认了内部非商用使用范围、许可证剩余风险、固定模型、安全边界、后续独立 Ticket 的受控下载授权、代表性测试机和资格门槛；该次授权当时不等同于门禁通过。受控取件、供应链证据、离线安全验证、资源与 `mock-eval-v1` 资格实测随后由 [Issue #39](https://github.com/transwarp-nc-finance-de/transwarp-service-insight/issues/39) 完成，并于 2026-07-19 获人工复核通过。

该工程建议置信度为**高**：结果绑定固定 revision、实际 SHA-256、镜像/依赖/SBOM、dataset checksum、测量 commit 和原始结果校验和。人工介入建议见“人工批准项”。待补充信息为生产批量索引可接受窗口、真实生产负载和多并发容量；这些信息不改变本次小样本工程资格结果，但限制其生产外推。
...
- `HISTORICAL`：2026-07-16 在完整证据产生前，Issue #19 的阶段状态为 `BLOCKED`；Issue #39 随后补齐证据，2026-07-19 最终人工结论为 `PASS`。
...
| `huggingface_hub` | Apache-2.0 | AutoTokenizer/AutoModel 的必要传递运行库；资格镜像保留库但删除 `hf` / `huggingface-cli` 下载入口。 | 固定传递版本与许可证；离线变量和 `network=none` 阻断远端能力，运行镜像不得包含下载 CLI 或 Token。 |
...
以上是工程侧许可证条款映射，不是正式法律意见。Issue #39 已生成最终 Python 依赖锁、镜像 SBOM 和第三方 NOTICE，并把三者 hash 绑定到资格运行；工程供应链门禁为 `PASS`。人工已复核许可证字段缺失项并接受训练数据权利链未逐项审计的剩余风险，但该确认不能被表述为正式法律意见。批准绑定模型 ID、40 位 revision、完整 SBOM、内部使用主体、镜像仓库、离线接收方范围与 NOTICE 包，而不是只批准模型名称。
...
| --- | --- | --- | --- |
| `model.safetensors` | 唯一权重文件 | 1,112,201,288 bytes | `a18a44fad1d0b46ded15928144138cff1135d5cc8233bdd90be5f18822de09a7`（实际交付字节） |
| `tokenizer.json` | 快速 tokenizer | 17,082,660 bytes | `62c24cdc13d4c9952d63718d6c9fa4c287974249e16b7ade6d5a85e7bbb75626`（实际交付字节） |
| `config.json` | XLM-R 模型架构、768 维等配置 | 694 bytes | `9dab198f24c8c0879e481cf7822005d5ecbceedbacb390ffafa594e28d31bac4`（实际交付字节） |
| `tokenizer_config.json` | tokenizer 类、长度等配置 | 418 bytes | `efb5c0d09722e5fe59a462cd2a9976ee216d55b037597d997cd3fe833216da15`（实际交付字节） |
| `special_tokens_map.json` | 特殊 Token 映射 | 280 bytes | `06e405a36dfe4b9604f484f6a1e619af1a7f7d09e34a8555eb0b77b66318067f`（实际交付字节） |

五个批准文件实际合计 1,129,285,340 bytes，规范化 manifest hash 为 `8f58395f4bf0f613fd15ebbf3d5467193ad6717715c4167c764e6e91aae1810e`，制品大小门禁为 `PASS`。资格制品明确排除 `sentencepiece.bpe.model`、`pytorch_model.bin`、`onnx/`、`openvino/`、TensorFlow 权重、`.gitattributes` 及其他重复或回退格式；若未来离线烟测证明 fast-tokenizer 清单不足，应停止而不是自动回退，并经人工评审后产生新 manifest。`README.md` 不进入运行时模型目录，但其固定 revision 快照、模型卡许可证元数据、本文和人工批准记录必须进入审计包。
...
**Issue #19 当前状态：`PASS`。** 2026-07-19 人工已完成报告、SBOM/NOTICE、manifest 和原始 checksum 复核并关闭 Issue。批准仅适用于固定模型、revision、manifest、依赖锁和既定运行边界；任一受控要素变化均须重新评估。模型制品不得提交 Git，产品构建必须校验批准的逐文件 SHA-256，运行时不得联网或隐式下载。

## 人工批准项

| 责任角色 | 需要明确批准的对象 | 当前状态 |
| --- | --- | --- |
| 法务/开源合规负责人 | 默认模型 MIT 标签及训练数据风险；企业内部非商用使用边界；LICENSE/NOTICE/SBOM 交付方式 | `CONFIRMED`；锁文件、SBOM 与 NOTICE 已人工复核，剩余风险已接受 |
| 信息安全/供应链负责人 | Hugging Face 固定 revision、Safetensors-only、禁止远程代码、内部网络和运行时禁止互联网出站；后续独立 Ticket 受控下载 | `CONFIRMED`；完整 SHA-256 manifest 与离线证据已生成 |
| 基础设施负责人 | 当前开发机为代表性主机；4 GB、CPU-only、查询 P95 门槛；批量索引只建基线 | `CONFIRMED`；实测工程门禁通过，生产窗口待确认 |
| 产品负责人 / SLA 处理人 / 安全负责人 | `mock-eval-v1` 四项门槛及“小样本工程评估”声明 | `CONFIRMED`；四项工程门禁通过，仍由人工确认 SLA 内容 |
| 架构负责人 | 默认模型 revision、768 维、512 Token、前缀/池化和模型/索引版本边界 | `CONFIRMED` |
| Service Insight Maintainers | 汇总上述签字、确认无事实冲突后，将 Issue #19 人工标为 PASS、FAIL 或继续 BLOCKED；SLA 内容及提交仍由人工确认 | `PASS`（2026-07-19，Issue 已关闭） |

## 审计来源

- [GitHub Issue #19](https://github.com/transwarp-nc-finance-de/transwarp-service-insight/issues/19)
- [后续受控取件与资格实测 Issue #39](https://github.com/transwarp-nc-finance-de/transwarp-service-insight/issues/39)
