...
**Issue #19 当前状态：`AWAITING_HUMAN_CONFIRMATION`。** Issue #39 已完成受控取件和工程资格实测，总体工程建议为 `PASS`；该建议不自动宣布 Decision Gate #19 通过，也没有修改默认 Compose 或产品运行时。
...
2026-07-16，用户以本项目唯一开发者及全部责任角色身份，已确认内部非商用使用范围、许可证剩余风险、固定模型、安全边界、后续独立 Ticket 的受控下载授权、代表性测试机和资格门槛；该授权不允许 Issue #19 本身下载模型，也不等同于门禁通过。受控取件、供应链证据、离线安全验证、资源与 `mock-eval-v1` 资格实测由 [Issue #39](https://github.com/transwarp-nc-finance-de/transwarp-service-insight/issues/39) 跟踪。
...
以上是工程侧许可证条款映射，不是正式法律意见。Issue #39 已生成最终 Python 依赖锁、镜像 SBOM 和第三方 NOTICE，并把三者 hash 绑定到资格运行；工程供应链门禁为 `PASS`，但许可证字段缺失项和训练数据权利链仍须人工复核，不能把工程结果表述为正式法律意见。法务批准必须绑定模型 ID、40 位 revision、完整 SBOM、内部使用主体、镜像仓库、离线接收方范围与 NOTICE 包，而不是只批准模型名称。
...
**Issue #19 当前状态：`AWAITING_HUMAN_CONFIRMATION`。** Issue #39 工程总体建议为 `PASS`，但建议保持 `ready-for-human`，不自动关闭门禁；只有人工复核报告、SBOM/NOTICE、manifest 和原始 checksum 后才能确认 `PASS/FAIL/BLOCKED`。模型不得打包进 Git、默认 Compose 或产品运行时。

## 人工批准项

| 责任角色 | 需要明确批准的对象 | 当前状态 |
| --- | --- | --- |
| 法务/开源合规负责人 | 默认模型 MIT 标签及训练数据风险；企业内部非商用使用边界；LICENSE/NOTICE/SBOM 交付方式 | `CONFIRMED`；锁文件、SBOM 与 NOTICE 已生成，仍需人工复核 |
...
| Service Insight Maintainers | 汇总上述签字、确认无事实冲突后，将 Issue #19 人工标为 PASS、FAIL 或继续 BLOCKED；SLA 内容及提交仍由人工确认 | `AWAITING_HUMAN_CONFIRMATION` |
