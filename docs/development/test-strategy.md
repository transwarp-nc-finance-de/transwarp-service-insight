...
仅验证必须依赖 PostgreSQL/pgvector 的行为。当前 PostgreSQL 专项覆盖空库迁移、`local-identity-v1`/`local-catalog-v1` 版本化种子、主外键与唯一约束、重复启动幂等、AuthSession 重启恢复，以及持久化预诊业务幂等键的真实并发串行化、三轮不可变 Run、完整度策略版本不可原地修改或删除和后端/PostgreSQL 重启恢复。查询前权限过滤、双索引原子切换、旧版本继续服务及 FTS/向量/RRF 排序待对应能力实现后增加；pgvector 当前不适用，不得以 Mock 或恒真断言伪造通过。
