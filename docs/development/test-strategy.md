...
当前阶段测试包括：后端单元与 API 行为测试、Spotless 格式检查、前端组件行为测试、ESLint、Prettier、生产构建、版本锁定的 OpenAPI 校验、三服务镜像构建及 Compose 冒烟。API 测试验证 v1 健康/预诊/追问/反馈、v2 AuthSession，以及知识上传、幂等、权限、ParseTask、预览、草稿修订与职责分离审核；领域测试覆盖结构优先切片、稳定哈希、PDF 文本层、重试上限和知识治理状态机；数据库测试验证跨版本并发幂等键串行化及草稿修订/审核历史的不可变写保护；组件测试覆盖模拟登录/切换/退出，以及编辑身份送审、审核身份按 Version ID 加载并批准或退回的流程；Compose 使用真实本地 PostgreSQL 和文件 volume 验证迁移与重启恢复。
