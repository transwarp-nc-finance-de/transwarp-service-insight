...
仅验证必须依赖 PostgreSQL/pgvector 的行为。当前专项覆盖空库迁移、版本化种子、约束、重复启动幂等、AuthSession/预诊恢复，以及 `vector(768)`、IndexTask 幂等/最多三次尝试、双索引原子切换、失败时旧版本继续服务和不可变尝试历史。在线查询前权限过滤与 FTS/向量/RRF 排序属于后续 Retrieval Ticket，不得在 #25 以 Mock 或恒真断言伪造通过。
