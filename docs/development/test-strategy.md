...
当前阶段测试包括：后端单元与 API 行为测试、Spotless 格式检查、前端组件行为测试、Playwright 浏览器纵向验收、ESLint、Prettier、生产构建、版本锁定的 OpenAPI 校验、四服务镜像构建及 Compose 冒烟。API 测试验证 v1 健康/预诊/追问/反馈、v2 AuthSession、知识治理，以及持久化预诊的创建、规范化幂等、多轮补充、上限、恢复、权限与显式终止；数据库测试验证并发幂等键串行化、知识审核历史和预诊 Run 快照的不可变写保护；组件测试覆盖模拟登录、知识审核和 v2 预诊恢复/补充/继续人工提交；Compose 使用真实本地 PostgreSQL、pgvector、固定离线 Embedding 和文件 volume 验证迁移、完整纵向闭环、故障降级与重启恢复。CI 在无模型制品的 FTS-only profile 中运行浏览器降级与 v1 回归；完整模型模式由 `tools/compose-acceptance/run.ps1` 在已批准制品上重复执行。
...
验证 Compose 服务健康、关键纵向闭环、容器重启持久化，以及显式 FTS 降级和运行中 Embedding 故障。当前完整四服务 Compose 必须同时回归无认证 v1，并验证 PostgreSQL/pgvector 迁移、固定制品真实离线 768 维推理、双索引发布、登录、数据库记录与原始知识文件哈希的重启恢复；显式 `compose.fts-only.yaml` 不启动 embedding 服务且应用仍健康。
