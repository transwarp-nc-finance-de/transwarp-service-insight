...
当前 `/api/v1` 保持兼容；一期新语义由 `/api/v2` 承载。v1/v2 DTO 通过 Mapper 隔离，领域模型不依赖 HTTP 版本。进入实现前必须形成完整 DRAFT OpenAPI，覆盖方法、请求/响应 Schema、错误码、幂等、分页、异步任务状态和 v1→v2 映射，并经人工确认；该门禁未完成前一期状态为 `NOT_READY`。本轮不修改已实现 OpenAPI。
