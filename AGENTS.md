...
仓库处于 `TECHNICAL_MVP`，聚焦 SLA 智能预诊助手。事实优先级为：用户当前明确指令 → `AGENTS.md` → `docs/project/scope.md` → 已实现 API 的 `docs/api/openapi.yaml` → `docs/architecture/current-architecture.md` → ACTIVE 文档 → DRAFT 设计 → ARCHIVED 历史材料。代码与文档冲突时必须显式报告，不得静默选择一方。
...
- 已实现 API 契约写入 `docs/api/openapi.yaml`，开发说明写入 `docs/development/`，运维治理写入 `docs/operations/`，历史材料写入 `docs/archive/`。
